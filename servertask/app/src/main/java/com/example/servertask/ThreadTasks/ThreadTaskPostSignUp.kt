...
class ThreadTaskPostSignUp : Thread {
    ...
    override fun run() {
        ...
        try {
            ...
            Log.w("com.example.servertask.MainActivity", "Inside run, connection open")
            connection.setDoOutput(true) //added by HF

            val os: OutputStream = connection.getOutputStream()
            Log.w("com.example.servertask.MainActivity", "Inside run after getting output stream")
            val osw = OutputStreamWriter(os)
            Log.w("com.example.servertask.MainActivity", "Inside run, before writing data")
            ...
            osw.write(data)
            osw.flush()
            Log.w("com.example.servertask.MainActivity", "Inside run, done posting")
            ...
            line = br.readLine()
            Log.w("com.example.servertask.MainActivity", "line is " + line)
